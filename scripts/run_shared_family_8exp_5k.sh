...
RUN_SCRIPT_DIR="outputs/run_scripts_shared_family_${RUN_TS}"
GENERATED_CFG_DIR="outputs/generated_configs_shared_family_${RUN_TS}"

EXPERIMENTS=(
  "full_shared_4x64_top1_alpha005_moe_only_5k|${CFG_DIR}/full_shared_4x64_top1_alpha005_moe_only_5k.json|sf_full_4x64_moeonly_5k|full_shared|moe_only|true|4|64|1|dense_prefix|random_ternary_matched|2816|0.05|0.0005|64|false|true"
  "full_shared_8x32_top2_alpha005_moe_only_5k|${CFG_DIR}/full_shared_8x32_top2_alpha005_moe_only_5k.json|sf_full_8x32_moeonly_5k|full_shared|moe_only|true|8|32|2|dense_prefix|random_ternary_matched|2816|0.05|0.0002|64|false|true"
  "full_shared_4x64_top1_alpha005_local_backbone_ft_5k|${CFG_DIR}/full_shared_4x64_top1_alpha005_local_backbone_ft_5k.json|sf_full_4x64_local_5k|full_shared|local_backbone_ft|true|4|64|1|dense_prefix|random_ternary_matched|2816|0.05|0.0005|64|false|true"
  "full_shared_8x32_top2_alpha005_local_backbone_ft_5k|${CFG_DIR}/full_shared_8x32_top2_alpha005_local_backbone_ft_5k.json|sf_full_8x32_local_5k|full_shared|local_backbone_ft|true|8|32|2|dense_prefix|random_ternary_matched|2816|0.05|0.0002|64|false|true"
  "shared_residual_topchannel_4x128_top1_discard_alpha025_isoarea_local_backbone_ft_5000step|${CFG_DIR}/shared_residual_topchannel_4x128_top1_discard_alpha025_isoarea_local_backbone_ft_5000step.json|sf_sr_4x128_local_5k|shared_residual|local_backbone_ft|true|4|128|1|dense_top_channel|dense_discarded_channel_split|2304|0.25|0.0005|128|true|false"
  "shared_residual_topchannel_4x128_top1_discard_alpha025_isoarea_moe_only_5000step|${CFG_DIR}/shared_residual_topchannel_4x128_top1_discard_alpha025_isoarea_moe_only_5000step.json|sf_sr_4x128_moeonly_5k|shared_residual|moe_only|true|4|128|1|dense_top_channel|dense_discarded_channel_split|2304|0.25|0.0005|128|true|false"
  "shared_residual_topchannel_8x64_top2_discard_alpha025_isoarea_local_backbone_ft_5000step|${CFG_DIR}/shared_residual_topchannel_8x64_top2_discard_alpha025_isoarea_local_backbone_ft_5000step.json|sf_sr_8x64_local_5k|shared_residual|local_backbone_ft|true|8|64|2|dense_top_channel|dense_discarded_channel_split|2304|0.25|0.0002|128|true|false"
  "shared_residual_topchannel_8x64_top2_discard_alpha025_isoarea_moe_only_5000step|${CFG_DIR}/shared_residual_topchannel_8x64_top2_discard_alpha025_isoarea_moe_only_5000step.json|sf_sr_8x64_moeonly_5k|shared_residual|moe_only|true|8|64|2|dense_top_channel|dense_discarded_channel_split|2304|0.25|0.0002|128|true|false"
)
...
write_config_payload() {
  ...
  local strict_total_param_fair="${14}"
  local skip_param_budget_resolver="${15}"
  local backbone_lr="0.00003"
  local norm_lr="0.00003"
  local shared_expert_lr="0.0001"
  local description=""
  ...
  if [[ "${family}" == "full_shared" ]]; then
    description="Full-shared residual experiment with exact shared width 2816, ${num_experts}x${sparse_expert_width} sparse residual experts, top${sparse_top_k}, alpha=${residual_scale_init}, freeze_mode=${freeze_mode}."
  else
    description="Shared-residual iso-area experiment with dense-top-channel shared path and discarded-channel sparse residual experts, ${num_experts}x${sparse_expert_width} top${sparse_top_k}, alpha=${residual_scale_init}, freeze_mode=${freeze_mode}."
  fi
  ...
  cat > "${target_cfg_path}" <<EOF
{
  ...
  "moe": {
    ...
    "auto_resolve_shared_width": $( [[ "${skip_param_budget_resolver}" == "true" ]] && echo "false" || echo "true" ),
    "min_shared_width": $( [[ "${family}" == "full_shared" ]] && echo "${nominal_shared_width}" || echo "2048" ),
    "shared_width_step": 16,
    "strict_total_param_fair": ${strict_total_param_fair},
    "skip_param_budget_resolver": ${skip_param_budget_resolver},
...
config_matches_spec() {
  ...
  local freeze_mode="$4"
  local num_experts="$5"
  local sparse_expert_width="$6"
  local sparse_top_k="$7"
  local shared_init="$8"
  local sparse_init="$9"
  local nominal_shared_width="${10}"
  local residual_scale_init="${11}"
  local router_aux_loss_coef="${12}"
  local strict_total_param_fair="${13}"
  local skip_param_budget_resolver="${14}"
  [[ -f "${cfg_path}" ]] || return 1
  "${PYTHON_CHECKER:-python3}" - "${cfg_path}" "${experiment_name}" "${family}" "${freeze_mode}" "${num_experts}" "${sparse_expert_width}" "${sparse_top_k}" "${shared_init}" "${sparse_init}" "${nominal_shared_width}" "${residual_scale_init}" "${router_aux_loss_coef}" "${strict_total_param_fair}" "${skip_param_budget_resolver}" <<'PY'
import json
import math
import sys
...
(
    ...
    expected_name,
    expected_family,
    expected_freeze_mode,
    expected_num_experts,
    expected_width,
    expected_topk,
    expected_shared_init,
    expected_sparse_init,
    expected_shared_width,
    expected_alpha,
    expected_aux,
    expected_strict_fair,
    expected_skip_resolver,
) = sys.argv[1:]
...
assert int(moe.get("nominal_shared_width", -1)) == int(expected_shared_width)
assert bool(moe.get("strict_total_param_fair", False)) == (expected_strict_fair == "true")
assert bool(moe.get("skip_param_budget_resolver", False)) == (expected_skip_resolver == "true")
assert moe.get("shared_init") == expected_shared_init
assert moe.get("sparse_init") == expected_sparse_init
...
if expected_family == "full_shared":
    assert bool(moe.get("auto_resolve_shared_width", True)) is False
else:
    assert bool(moe.get("auto_resolve_shared_width", False)) is True
PY
}
...
prepare_configs() {
  ...
  for spec in "${EXPERIMENTS[@]}"; do
    IFS='|' read -r experiment_name cfg_path _ family freeze_mode enable_sparse_residual num_experts sparse_expert_width sparse_top_k shared_init sparse_init nominal_shared_width residual_scale_init router_aux_loss_coef _ strict_total_param_fair skip_param_budget_resolver <<< "${spec}"
    if config_matches_spec "${cfg_path}" "${experiment_name}" "${family}" "${freeze_mode}" "${num_experts}" "${sparse_expert_width}" "${sparse_top_k}" "${shared_init}" "${sparse_init}" "${nominal_shared_width}" "${residual_scale_init}" "${router_aux_loss_coef}" "${strict_total_param_fair}" "${skip_param_budget_resolver}" >/dev/null 2>&1; then
    ...
    write_config_payload \
      ...
      "${shared_init}" \
      "${sparse_init}" \
      "${nominal_shared_width}" \
      "${residual_scale_init}" \
      "${router_aux_loss_coef}" \
      "${strict_total_param_fair}" \
      "${skip_param_budget_resolver}"
...
if missing:
    raise SystemExit("Required shared-family capability checks failed:\n- " + "\n- ".join(missing))
print("Shared-family capability checks passed.")
PY
}

safety_checks() {
  cd "${WORKDIR}"
  [[ ${#GPU_LIST[@]} -eq 8 ]] || die "GPU_MAP must contain exactly 8 comma-separated GPU ids. Got: ${GPU_MAP}"
  LAUNCHABLE_EXPERIMENTS=()

  for idx in "${!EXPERIMENTS[@]}"; do
    IFS='|' read -r experiment_name _ session_name family freeze_mode _ num_experts sparse_expert_width sparse_top_k shared_init sparse_init nominal_shared_width residual_scale_init router_aux_loss_coef _ strict_total_param_fair skip_param_budget_resolver <<< "${EXPERIMENTS[$idx]}"
    ...
    config_matches_spec "${cfg_path}" "${experiment_name}" "${family}" "${freeze_mode}" "${num_experts}" "${sparse_expert_width}" "${sparse_top_k}" "${shared_init}" "${sparse_init}" "${nominal_shared_width}" "${residual_scale_init}" "${router_aux_loss_coef}" "${strict_total_param_fair}" "${skip_param_budget_resolver}" >/dev/null \
      || die "Resolved config does not match expected semantics: ${cfg_path}"
...
run_preflight() {
  ...
  for idx in "${!EXPERIMENTS[@]}"; do
    IFS='|' read -r experiment_name _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ <<< "${EXPERIMENTS[$idx]}"
...
create_launch_scripts() {
  ...
  mkdir -p "${RUN_SCRIPT_DIR}"
  for idx in "${!EXPERIMENTS[@]}"; do
    IFS='|' read -r experiment_name _ session_name _ _ _ _ _ _ _ _ _ _ _ _ _ _ <<< "${EXPERIMENTS[$idx]}"
    [[ "${EXP_STATUS[${experiment_name}]:-skip}" == "launch" ]] || continue
    local cfg_path="${EXP_CFG_PATH[${experiment_name}]:-}"
    local gpu="${GPU_LIST[$idx]}"
...
print_monitoring_commands() {
  ...
  echo "  nvidia-smi"
  for spec in "${EXPERIMENTS[@]}"; do
    IFS='|' read -r experiment_name _ session_name _ _ _ _ _ _ _ _ _ _ _ _ _ _ <<< "${spec}"
...
specs = [
    {
        ...
        "strict_total_param_fair_passed": False,
        "resolved_shared_width": 2816,
    },
    {
    ...
    },
    {
        ...
        "strict_total_param_fair_passed": True,
        "resolved_shared_width": None,
    },
    {
...
for spec in specs:
    ...
    if spec["family"] == "shared_residual":
        ...
        if int(budget.get("resolved_shared_width", -1)) != int(spec["resolved_shared_width"]):
            raise SystemExit(f"{spec['experiment_name']}: resolved_shared_width mismatch")
